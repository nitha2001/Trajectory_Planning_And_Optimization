...
def find_throwing_trajectory(N, initial_state, final_configuration, distance, tf):
  ...
  cost = 0
  for i in range(N-1):
      cost += 0.5*(timesteps[i+1]-timesteps[i])*(u[i].T@u[i] + u[i+1].T@u[i+1])
  print('cost is',cost)
  ...
  for i in range(N):
      ...
      prog.AddBoundingBoxConstraint(-joint_limits, joint_limits, x[i,0:n_q])
  ...
  u_v = np.random.uniform(-effort_limits, effort_limits,(N,n_u))
  prog.SetInitialGuess(u, u_v)
  
  q_v = np.random.uniform(-joint_limits, joint_limits,(N,n_q))
  qdot_v = np.random.uniform(-joint_limits, joint_limits,(N,n_q))

  
  for i in range(N):
      prog.SetInitialGuess(x[i,0:n_q], q_v[i])
      prog.SetInitialGuess(x[i,n_q:], qdot_v[i])
