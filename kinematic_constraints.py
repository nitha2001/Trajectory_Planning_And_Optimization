...
def landing_constraint(vars):
  ...
  constraint_eval[0] = pos[1] + t_land * vel[1] - (0.5 * g * t_land**2)
  # constraint_eval[1]: Eq (24)
  constraint_eval[1] = pos[0] + t_land * vel[0]
  # constraint_eval[2]: Eq (26)
  constraint_eval[2] = pos[1]   #is this tf or tland?
...
def AddFinalLandingPositionConstraint(prog, xf, d, t_land):
  ...
  lb = np.array([0,d,2])
  ub = np.array([0,d,2])
  v  = np.hstack((xf,t_land[0]))
  prog.AddConstraint(landing_constraint, lb, ub, v)
  # TODO: Add a constraint that t_land is positive
  # prog.AddBoundingBoxConstraint(0.0, np.inf, t_land[0]) # Could be an issue check again 
  prog.AddConstraint(t_land[0]>=0.00000001)